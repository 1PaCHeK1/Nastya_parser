...
remove_favorite_keyboard = (
    types.InlineKeyboardMarkup()
    .add(types.InlineKeyboardButton(texts.delete_from_favourites_text, callback_data=CallbakDataEnum.remove_favorite.value))
)

no_auth_start_keyboard = (
    types.InlineKeyboardMarkup()
    .add(types.InlineKeyboardButton("Регистрация", callback_data=CallbakDataEnum.registration.value))
)
